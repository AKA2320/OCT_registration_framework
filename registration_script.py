...
class RegistrationMaster:
    def __init__(
        ...
        USE_MODEL_LATERAL_TRANSLATION=False,
        ...
        if not self.is_gui_flag:
            ...
            logging.basicConfig(
                level=logging.INFO,
                format="%(message)s",
                stream=ProcessStdout(self.output_queue),
            )
        ...
        self.MODEL_FEATURE_DETECT_PATH = self.config["PATHS"][
            "MODEL_FEATURE_DETECT_PATH"
        ]
        self.MODEL_X_TRANSLATION_PATH = self.config["PATHS"]["MODEL_X_TRANSLATION_PATH"]
        self.DISABLE_TQDM = self.config["FLAGS"]["DISABLE_TQDM"]
        self.USE_MODEL_LATERAL_TRANSLATION = self.config["FLAGS"][
            "USE_MODEL_LATERAL_TRANSLATION"
        ]
    ...
    def _set_config(
        ...
        try:
            config["PATHS"]["MODEL_FEATURE_DETECT_PATH"] = resource_path(
                config["PATHS"]["MODEL_FEATURE_DETECT_PATH"]
            )
            config["PATHS"]["MODEL_X_TRANSLATION_PATH"] = resource_path(
                config["PATHS"]["MODEL_X_TRANSLATION_PATH"]
            )
        except:
            config["PATHS"]["MODEL_FEATURE_DETECT_PATH"] = config["PATHS"][
                "MODEL_FEATURE_DETECT_PATH"
            ]
            config["PATHS"]["MODEL_X_TRANSLATION_PATH"] = config["PATHS"][
                "MODEL_X_TRANSLATION_PATH"
            ]
        return config
        ...
        if self.USE_MODEL_LATERAL_TRANSLATION:
            try:
                if not os.path.exists(self.MODEL_X_TRANSLATION_PATH):
                    logging.info(
                        "Model X not present in models.....Downloading the model."
                    )
                    download_model(
                        model_x_translation_url, self.MODEL_X_TRANSLATION_PATH
                    )
    ...
    def spawn_worker_and_run_pipeline(self):
        ...
        if not self.BATCH_FLAG:
            ...
            scans = [
                i for i in os.listdir(self.DATA_LOAD_DIR) if (i.startswith("scan"))
            ]
    ...
    def _dask_slurm_spawner(self, scans):
        try:
            ...
            raise ImportError(
                "Dask and dask_jobqueue modules are required for multiprocessing with SLURM"
            ) from e
        ...
        tasks = [
            delayed(self._launch_process_wrapper)(args) for args in multiproc_args_list
        ]
...
def start_reg_cli():
    registration_pipeline = RegistrationMaster(
        config_path="datapaths.yaml", is_gui=False
    )
