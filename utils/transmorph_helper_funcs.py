...
def filter_list(result_list, expected_num):
    ...
    for group in grouped.values():
        top_two = sorted(group, key=lambda x: x["confidence"], reverse=True)[
            :expected_num
        ]
...
def detect_areas(result_list, pad_val, img_shape, expected_num=2):
    ...
    for detections in result_list:
        coords.append(
            [
                int(detections["box"]["y1"]) - pad_val,
                int(detections["box"]["y2"]) + pad_val,
            ]
        )
    if len(coords) == 0:
        return None
...
def crop_data(data, surface_coords, cells_coords, max_crop_shape):
    ...
    if surface_coords is not None:
        surface_coords[:, 0], surface_coords[:, 1] = (
            surface_coords[:, 0] - 30,
            surface_coords[:, 1] + 30,
        )  # 30 is for padding, makes it atleast 60 pixels for transmorph to work
        surface_coords = np.where(surface_coords < 0, 0, surface_coords)
        surface_coords = np.where(
            surface_coords > max_crop_shape, max_crop_shape - 1, surface_coords
        )
        merged_coords.extend([*surface_coords])
    if cells_coords is not None:
        cells_coords[:, 0], cells_coords[:, 1] = (
            cells_coords[:, 0] - 30,
            cells_coords[:, 1] + 30,
        )  # 30 is for padding, makes it atleast 60 pixels for transmorph to work
        cells_coords = np.where(cells_coords < 0, 0, cells_coords)
        cells_coords = np.where(
            cells_coords > max_crop_shape, max_crop_shape - 1, cells_coords
        )
        merged_coords.extend([*cells_coords])
    merged_coords = merge_intervals([*merged_coords])
    uncroped_data = uncroped_data[
        :, np.r_[tuple(np.r_[start:end] for start, end in merged_coords)], :
    ]
...
class CropOrPad:
    def __init__(self, target_shape: tuple):
        if not isinstance(target_shape, (tuple, list)) or len(target_shape) != 2:
            raise ValueError(
                "target_shape must be a tuple or list of two integers (height, width)."
            )
    ...
    def __call__(self, img: torch.Tensor) -> torch.Tensor:
        ...
        if img.dim() == 2:  # (H, W) grayscale
            ...
            raise ValueError(
                f"Unsupported image tensor dimensions: {img.dim()}. Expected 2 or 3."
            )
        ...
        if (
            current_height_padded > self.target_height
            or current_width_padded > self.target_width
        ):
        ...
        if is_grayscale:
            img = img.squeeze(
                0
            )  # Remove the channel dimension if it was grayscale initially
...
def infer_x_translation(model_obj, static_np, moving_np, DEVICE="cpu"):
    ...
    with torch.no_grad():
        input_pair = (
            torch.cat([static_np, moving_np], dim=1).double().to(DEVICE)
        )  # shape: (1, 2, H, W)
        _, pred_translation = model_obj(input_pair)

        input_pair_rev = (
            torch.cat([moving_np, static_np], dim=1).double().to(DEVICE)
        )  # shape: (1, 2, H, W)
        _, pred_translation_rev = model_obj(input_pair_rev)
    return (
        pred_translation.squeeze().numpy()[0],
        pred_translation_rev.squeeze().numpy()[0],
    )
