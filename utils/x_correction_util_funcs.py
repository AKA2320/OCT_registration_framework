...
def get_line_shift(line_1d_stat, line_1d_mov):
    ...
    for _ in range(7):
        move = minz(
            ...
            x0=np.array([0.0]),
            bounds=[(-4, 4)],
            args=(line_1d_stat, line_1d_mov, past_shift),
        )["x"]
        past_shift += move[0]
    return (
        -past_shift * 2
    )  # Negative because scipy shift returns opposite direction shift
...
def get_cell_patch_shift(patch_stat, patch__mov):
    ...
    for _ in range(7):
        move = minz(
            ...
            x0=np.array([0.0]),
            bounds=[(-4, 4)],
            args=(patch_stat, patch__mov, past_shift),
        )["x"]
        past_shift += move[0]
    return past_shift * 2
...
def compute_transform_for_pair(
    i, static_img, moving_img, cells_coords, enface_extraction_rows, valid_args
):
    ...
    if len(enface_extraction_rows) > 0:
        ...
        # else:
        enface_wraps = x_correction_enface_manual(
            static_img, moving_img, enface_extraction_rows
        )
    else:
...
def x_correction_cell_manual(static_img, moving_img, cells_coords):
    if cells_coords.shape[0] == 1:
        ...
        stat = static_img[
            np.r_[tuple(np.r_[start:end] for start, end in cells_coords)], :
        ]
        temp_manual = moving_img[
            np.r_[tuple(np.r_[start:end] for start, end in cells_coords)], :
        ]
    # MANUAL
