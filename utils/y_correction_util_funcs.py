...
def y_motion_correcting(
    data, slice_coords, top_surf, partition_coord, disable_tqdm, scan_num
):
    ...
    if partition_coord is None:
        ...
        for i in tqdm(
            ...
            data[i, :partition_coord] = warp_image_affine(
                data[i, :partition_coord], (0, -tr_y[i, 1, 2])
            )
    else:
