...
class RegistrationWorker:
    ...
    def process_scan(self):
        ...
        try:
            ...
            self.surface_coords, self.partition_coord = (
                self._get_surface_coords_cropped(cropped_data)
            )
            if self.surface_coords is None:
                logging.warning(
                    f"No surface detected in cropped data, cropping error: {self.scan_num}"
                )
                return None
    ...
    def _detect_features_crop_data(self, original_data):
        ...
        res_surface_cells = self.MODEL_FEATURE_DETECT.predict(
            test_detect_img,
            iou=0.5,
            save=self.save_detections,
            max_det=self.EXPECTED_SURFACES
            + self.EXPECTED_CELLS
            + 5,  # some extra detctions to save but not use
            project=detections_save_dir,
            name=self.scan_num,
            verbose=False,
            classes=0,
            device=self.DEVICE,
        ...
        res_surface = self.MODEL_FEATURE_DETECT.predict(
            test_detect_img,
            iou=0.5,
            save=False,
            max_det=self.EXPECTED_SURFACES,
        ...
        res_cells = self.MODEL_FEATURE_DETECT.predict(
            test_detect_img,
            iou=0.5,
            save=False,
            max_det=self.EXPECTED_CELLS,
            verbose=False,
            classes=1,
            device=self.DEVICE,
            agnostic_nms=True,
        ...
        surface_crop_coords = detect_areas(
            ...
            pad_val=self.SURFACE_Y_PAD,
            img_shape=test_detect_img.shape[0],
            expected_num=self.EXPECTED_SURFACES,
        )
        cells_crop_coords = detect_areas(
            res_cells[0].summary(),
            pad_val=self.SURFACE_Y_PAD,
            img_shape=test_detect_img.shape[0],
            expected_num=self.EXPECTED_CELLS,
        )
        if surface_crop_coords is None:
            logging.warning(
                f"No surface detected in orignal data for cropping: {self.scan_num}"
            )
            return None
        return crop_data(
            original_data,
    ...
    def _get_surface_coords_cropped(self, cropped_data):
        ...
        if self.EXPECTED_SURFACES > 1:
            ...
            partition_coord = np.ceil(
                np.mean(np.mean(surface_coords[-2:], axis=1))
            ).astype(int)
        else:
    ...
    def _correct_x_motion(self, data):
        ...
        static_flat = np.argmax(
            np.sum(
                data[:, self.surface_coords[0, 0] : self.surface_coords[0, 1], :],
                axis=(0, 1),
            )
        )
        ...
        surface_coords_for_x = detect_areas(
            res_surface[0].summary(),
            pad_val=self.SURFACE_X_PAD,
            img_shape=test_detect_img.shape[0],
            expected_num=self.EXPECTED_SURFACES,
        )
        cells_coords_for_x = detect_areas(
            res_cells[0].summary(),
            pad_val=self.CELLS_X_PAD,
            img_shape=test_detect_img.shape[0],
            expected_num=self.EXPECTED_CELLS,
        )
        ...
        if surface_coords_for_x is not None:
            static_y_motion = np.argmax(
                np.sum(
                    data[:, surface_coords_for_x[0, 0] : surface_coords_for_x[0, 1], :],
                    axis=(1, 2),
                )
            )
            errs = []
            ...
            for i in range(surface_coords_for_x.shape[0]):
                val = np.argmax(
                    np.sum(
                        np.max(
                            data[
                                :,
                                surface_coords_for_x[i, 0] : surface_coords_for_x[i, 1],
                                :,
                            ],
                            axis=0,
                        ),
                ...
                enface_extraction_rows.append(
                    surface_coords_for_x[i, 0] + val
                )  # val is argmax so always u32 and surface_coords_for_x is u32 check detect_areas()
        else:
            valid_args = np.arange(data.shape[0])

        tr_all = x_motion_correction(
            data,
            cells_coords_for_x,
            valid_args,
            enface_extraction_rows,
            self.DISABLE_TQDM,
            self.scan_num,
            self.MODEL_X_TRANSLATION_PATH,
        )

        for i in tqdm(
            range(1, data.shape[0], 2),
            desc="X-motion warping",
    ...
    def _save_data(self, data, suffix=""):
        ...
        with h5py.File(hdf5_filename, "w") as hf:
            hf.create_dataset(
                "volume", data=data, compression="gzip", compression_opts=5
            )
